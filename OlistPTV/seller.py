...
class Seller:
    ...
    def get_training_data(self):
        ...
        training_set['revenue_from_sales'] = training_set.sales * sales_cut

        training_set['active_months'] =\
            ((training_set.date_last_sale - training_set.date_first_sale) / np.timedelta64(1, 'M'))\
                .map(lambda x: x if x > 0 else 1)
        training_set['active_months'] = np.ceil(training_set['active_months'])

        training_set['revenue_from_monthly_fees'] = training_set['active_months'] * monthly_fee

        training_set['revenues'] = training_set['revenue_from_sales'] + training_set['revenue_from_monthly_fees']

        training_set['profit'] = training_set.revenues - training_set.cost_of_reviews
